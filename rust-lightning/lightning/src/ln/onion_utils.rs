...
use util::ser::{LengthCalculatingWriter, Readable, Writeable};
...
#[inline]
pub(super) fn construct_onion_keys_callback<
	T: secp256k1::Signing,
	FType: FnMut(SharedSecret, [u8; 32], PublicKey, &RouteHop, usize),
>(
	secp_ctx: &Secp256k1<T>, path: &Vec<RouteHop>, session_priv: &SecretKey, mut callback: FType,
...
pub(super) fn construct_onion_keys<T: secp256k1::Signing>(
	secp_ctx: &Secp256k1<T>, path: &Vec<RouteHop>, session_priv: &SecretKey,
...
pub(super) fn build_onion_payloads(
	path: &Vec<RouteHop>, total_msat: u64, payment_secret_option: &Option<PaymentSecret>,
	starting_htlc_offset: u32, keysend_preimage: &Option<PaymentPreimage>,
) -> Result<(Vec<msgs::OnionHopData>, u64, u32), APIError> {
		...
		res.insert(
			...
			msgs::OnionHopData {
				format: if hop.node_features.supports_variable_length_onion() {
					...
					msgs::OnionHopDataFormat::Legacy { short_channel_id: last_short_channel_id }
				},
...
pub(super) fn construct_onion_packet(
	payloads: Vec<msgs::OnionHopData>, onion_keys: Vec<OnionKeys>, prng_seed: [u8; 32],
...
// msgs::OnionHopData.
pub(super) fn construct_onion_packet_bogus_hopdata<HD: Writeable>(
...
/// panics if route_size_insane(paylods)
fn construct_onion_packet_with_init_noise<HD: Writeable>(
	mut payloads: Vec<HD>, onion_keys: Vec<OnionKeys>, mut packet_data: [u8; ONION_DATA_LEN],
	associated_data: &PaymentHash,
) -> msgs::OnionPacket {
	let filler = {
	...
	L::Target: Logger,
{
	...
	} = htlc_source
	{
		...
		construct_onion_keys_callback(
			...
			path,
			session_priv,
			|shared_secret, _, _, route_hop, route_hop_idx| {
				if res.is_some() {
					return;
				}
					...
					msgs::DecodedOnionErrorPacket::read(&mut Cursor::new(&packet_decrypted))
				{
					...
					if fixed_time_eq(&Hmac::from_engine(hmac).into_inner(), &err_packet.hmac) {
						if let Some(error_code_slice) = err_packet.failuremsg.get(0..2) {
							...
							let (debug_field, debug_field_size) =
								errors::get_onion_debug_field(error_code);
							...
							if error_code & NODE == NODE {
								...
								if let Some(update_len_slice) = err_packet
									.failuremsg
									.get(debug_field_size + 2..debug_field_size + 4)
								{
									let update_len = u16::from_be_bytes(
										update_len_slice.try_into().expect("len is 2"),
									) as usize;
									if let Some(update_slice) = err_packet.failuremsg.get(
										debug_field_size + 4..debug_field_size + 4 + update_len,
									) {
										if let Ok(chan_update) = msgs::ChannelUpdate::read(
											&mut Cursor::new(&update_slice),
										) {
											...
											let is_chan_update_invalid = match error_code & 0xff {
												...
												11 => {
													amt_to_forward
														> chan_update.contents.htlc_minimum_msat
												}
												12 => amt_to_forward
													.checked_mul(
														chan_update
													...
													.and_then(|prop_fee| {
														prop_fee.checked_add(
															chan_update.contents.fee_base_msat
																as u64,
														)
													})
													.map(|fee_msats| {
														route_hop.fee_msat >= fee_msats
													})
													.unwrap_or(false),
												13 => {
													route_hop.cltv_expiry_delta as u16
														>= chan_update.contents.cltv_expiry_delta
												}
											...
											if is_chan_update_invalid {
												...
												{
													short_channel_id =
														Some(failing_route_hop.short_channel_id);
												}
												network_update =
							...
							let (description, title) =
								errors::get_onion_error_description(error_code);
							if debug_field_size > 0
								&& err_packet.failuremsg.len() >= 4 + debug_field_size
							{
								log_warn!(
									logger,
									"Onion Error[from {}: {}({:#x}) {}({})] {}",
	...
	// is_from_final_node -> payment_retryable -> rejected_by_dest
	} else {
		unreachable!();
	}
}
...
mod tests {
	...
	fn build_test_onion_keys() -> Vec<OnionKeys> {
		...
		let session_priv = SecretKey::from_slice(
			&hex::decode("4141414141414141414141414141414141414141414141414141414141414141")
				.unwrap()[..],
		)
		.unwrap();

		let onion_keys =
			super::construct_onion_keys(&secp_ctx, &route.paths[0], &session_priv).unwrap();
	...
	fn onion_vectors() {
		...
		assert_eq!(
			...
			hex::decode("53eb63ea8a3fec3b3cd433b85cd62a4b145e1dda09391b348c4e1cd36a03ea66")
				.unwrap()[..]
		);
		assert_eq!(
			onion_keys[0].blinding_factor[..],
			hex::decode("2ec2e5da605776054187180343287683aa6a51b4b1c04d6dd49c45d8cffb3c36")
				.unwrap()[..]
		);
		assert_eq!(
		...
		assert_eq!(
			onion_keys[0].rho,
			hex::decode("ce496ec94def95aadd4bec15cdb41a740c9f2b62347c4917325fcc6fb0453986")
				.unwrap()[..]
		);
		assert_eq!(
			onion_keys[0].mu,
			hex::decode("b57061dc6d0a2b9f261ac410c8b26d64ac5506cbba30267a649c28c179400eba")
				.unwrap()[..]
		);

		assert_eq!(
			onion_keys[1].shared_secret[..],
			hex::decode("a6519e98832a0b179f62123b3567c106db99ee37bef036e783263602f3488fae")
				.unwrap()[..]
		);
		assert_eq!(
			onion_keys[1].blinding_factor[..],
			hex::decode("bf66c28bc22e598cfd574a1931a2bafbca09163df2261e6d0056b2610dab938f")
				.unwrap()[..]
		);
		assert_eq!(
			onion_keys[1].ephemeral_pubkey.serialize()[..],
			hex::decode("028f9438bfbf7feac2e108d677e3a82da596be706cc1cf342b75c7b7e22bf4e6e2")
				.unwrap()[..]
		);
		assert_eq!(
			onion_keys[1].rho,
			hex::decode("450ffcabc6449094918ebe13d4f03e433d20a3d28a768203337bc40b6e4b2c59")
				.unwrap()[..]
		);
		assert_eq!(
			onion_keys[1].mu,
			hex::decode("05ed2b4a3fb023c2ff5dd6ed4b9b6ea7383f5cfe9d59c11d121ec2c81ca2eea9")
				.unwrap()[..]
		);

		assert_eq!(
			onion_keys[2].shared_secret[..],
			hex::decode("3a6b412548762f0dbccce5c7ae7bb8147d1caf9b5471c34120b30bc9c04891cc")
				.unwrap()[..]
		);
		assert_eq!(
			onion_keys[2].blinding_factor[..],
			hex::decode("a1f2dadd184eb1627049673f18c6325814384facdee5bfd935d9cb031a1698a5")
				.unwrap()[..]
		);
		assert_eq!(
			onion_keys[2].ephemeral_pubkey.serialize()[..],
			hex::decode("03bfd8225241ea71cd0843db7709f4c222f62ff2d4516fd38b39914ab6b83e0da0")
				.unwrap()[..]
		);
		assert_eq!(
			onion_keys[2].rho,
			hex::decode("11bf5c4f960239cb37833936aa3d02cea82c0f39fd35f566109c41f9eac8deea")
				.unwrap()[..]
		);
		assert_eq!(
			onion_keys[2].mu,
			hex::decode("caafe2820fa00eb2eeb78695ae452eba38f5a53ed6d53518c5c6edf76f3f5b78")
				.unwrap()[..]
		);

		assert_eq!(
			onion_keys[3].shared_secret[..],
			hex::decode("21e13c2d7cfe7e18836df50872466117a295783ab8aab0e7ecc8c725503ad02d")
				.unwrap()[..]
		);
		assert_eq!(
			onion_keys[3].blinding_factor[..],
			hex::decode("7cfe0b699f35525029ae0fa437c69d0f20f7ed4e3916133f9cacbb13c82ff262")
				.unwrap()[..]
		);
		assert_eq!(
			onion_keys[3].ephemeral_pubkey.serialize()[..],
			hex::decode("031dde6926381289671300239ea8e57ffaf9bebd05b9a5b95beaf07af05cd43595")
				.unwrap()[..]
		);
		assert_eq!(
			onion_keys[3].rho,
			hex::decode("cbe784ab745c13ff5cffc2fbe3e84424aa0fd669b8ead4ee562901a4a4e89e9e")
				.unwrap()[..]
		);
		assert_eq!(
			onion_keys[3].mu,
			hex::decode("5052aa1b3d9f0655a0932e50d42f0c9ba0705142c25d225515c45f47c0036ee9")
				.unwrap()[..]
		);

		assert_eq!(
			onion_keys[4].shared_secret[..],
			hex::decode("b5756b9b542727dbafc6765a49488b023a725d631af688fc031217e90770c328")
				.unwrap()[..]
		);
		assert_eq!(
			onion_keys[4].blinding_factor[..],
			hex::decode("c96e00dddaf57e7edcd4fb5954be5b65b09f17cb6d20651b4e90315be5779205")
				.unwrap()[..]
		);
		assert_eq!(
			onion_keys[4].ephemeral_pubkey.serialize()[..],
			hex::decode("03a214ebd875aab6ddfd77f22c5e7311d7f77f17a169e599f157bbcdae8bf071f4")
				.unwrap()[..]
		);
		assert_eq!(
			onion_keys[4].rho,
			hex::decode("034e18b8cc718e8af6339106e706c52d8df89e2b1f7e9142d996acf88df8799b")
				.unwrap()[..]
		);
		assert_eq!(
			onion_keys[4].mu,
			hex::decode("8e45e5c61c2b24cb6382444db6698727afb063adecd72aada233d4bf273d975a")
				.unwrap()[..]
		);
		...
		let payloads = vec![
			msgs::OnionHopData {
				format: msgs::OnionHopDataFormat::Legacy { short_channel_id: 0 },
				amt_to_forward: 0,
				outgoing_cltv_value: 0,
			},
			msgs::OnionHopData {
				format: msgs::OnionHopDataFormat::Legacy { short_channel_id: 0x0101010101010101 },
				amt_to_forward: 0x0100000001,
				outgoing_cltv_value: 0,
			},
			msgs::OnionHopData {
				format: msgs::OnionHopDataFormat::Legacy { short_channel_id: 0x0202020202020202 },
				amt_to_forward: 0x0200000002,
				outgoing_cltv_value: 0,
			},
			msgs::OnionHopData {
				format: msgs::OnionHopDataFormat::Legacy { short_channel_id: 0x0303030303030303 },
		...
		let packet = super::construct_onion_packet_with_init_noise(
			payloads,
			onion_keys,
			[0; super::ONION_DATA_LEN],
			&PaymentHash([0x42; 32]),
		);
	...
	fn test_failure_packet_onion() {
		...
		let onion_keys = build_test_onion_keys();
		let onion_error =
			super::build_failure_packet(&onion_keys[4].shared_secret[..], 0x2002, &[0; 0]);
