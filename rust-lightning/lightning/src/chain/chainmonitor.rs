...
use prelude::*;
use sync::{Mutex, MutexGuard, RwLock, RwLockReadGuard};
...
impl MonitorUpdateId {
	...
	pub(crate) fn from_new_monitor<ChannelSigner: Sign>(
...
pub trait Persist<ChannelSigner: Sign> {
	...
	fn persist_new_channel(
		&self, channel_id: OutPoint, data: &ChannelMonitor<ChannelSigner>,
...
impl<ChannelSigner: Sign> MonitorHolder<ChannelSigner> {
	fn has_pending_offchain_updates(
		&self, pending_monitor_updates_lock: &MutexGuard<Vec<MonitorUpdateId>>,
	) -> bool {
		pending_monitor_updates_lock.iter().any(|update_id| {
			if let UpdateOrigin::OffChain(_) = update_id.contents {
				true
			} else {
				false
			}
	...
	fn has_pending_chainsync_updates(
		...
		pending_monitor_updates_lock.iter().any(|update_id| {
			if let UpdateOrigin::ChainSync(_) = update_id.contents {
				true
			} else {
				false
			}
	...
	P::Target: Persist<ChannelSigner>,
{
				...
				let mut txn_outputs;
				{
					...
					match self.persister.update_persisted_channel(
						*funding_outpoint,
						...
						Ok(()) => log_trace!(
							self.logger,
							"Finished syncing Channel Monitor for channel {}",
							log_funding_info!(monitor)
						),
						Err(ChannelMonitorUpdateErr::PermanentFailure) => {
							...
							self.pending_monitor_events
								.lock()
								.unwrap()
								.push(MonitorEvent::UpdateFailed(*funding_outpoint));
						}
	...
	pub fn channel_monitor_updated(
		&self, funding_txo: OutPoint, completed_update_id: MonitorUpdateId,
	) -> Result<(), APIError> {
		...
		match completed_update_id {
			MonitorUpdateId { contents: UpdateOrigin::OffChain(_) } => {
				...
				// complete.
				let monitor_is_pending_updates =
					monitor_data.has_pending_offchain_updates(&pending_monitor_updates);
				if monitor_is_pending_updates
					|| monitor_data.channel_perm_failed.load(Ordering::Acquire)
				{
			...
			MonitorUpdateId { contents: UpdateOrigin::ChainSync(_) } => {
				if !monitor_data.has_pending_chainsync_updates(&pending_monitor_updates) {
					monitor_data.last_chain_persist_height.store(
	...
	pub fn force_channel_monitor_updated(&self, funding_txo: OutPoint, monitor_update_id: u64) {
		self.pending_monitor_events
			.lock()
			.unwrap()
			.push(MonitorEvent::UpdateCompleted { funding_txo, monitor_update_id });
	}
...
impl<ChannelSigner: Sign, C: Deref, T: Deref, F: Deref, L: Deref, P: Deref> chain::Listen
	for ChainMonitor<ChannelSigner, C, T, F, L, P>
where
	C::Target: chain::Filter,
	T::Target: BroadcasterInterface,
	F::Target: FeeEstimator,
...
impl<ChannelSigner: Sign, C: Deref, T: Deref, F: Deref, L: Deref, P: Deref> chain::Confirm
	for ChainMonitor<ChannelSigner, C, T, F, L, P>
where
	C::Target: chain::Filter,
	T::Target: BroadcasterInterface,
	...
	P::Target: Persist<ChannelSigner>,
{
	...
	fn watch_channel(
		...
		log_trace!(
			self.logger,
			"Got new ChannelMonitor for channel {}",
			log_funding_info!(monitor)
		);
		...
		if persist_res.is_err() {
			log_error!(
				self.logger,
				"Failed to persist new ChannelMonitor for channel {}: {:?}",
				log_funding_info!(monitor),
				persist_res
			);
		} else {
			log_trace!(
				self.logger,
				"Finished persisting new ChannelMonitor for channel {}",
				log_funding_info!(monitor)
			);
		}
		...
		entry.insert(MonitorHolder {
			...
			last_chain_persist_height: AtomicUsize::new(
				self.highest_chain_height.load(Ordering::Acquire),
			),
		});
		persist_res
	...
	fn update_channel(
		...
		match monitors.get(&funding_txo) {
			None => {
				log_error!(
					self.logger,
					"Failed to update channel monitor: no such monitor registered"
				);
			...
			Some(monitor_state) => {
				...
				log_trace!(
					self.logger,
					"Updating ChannelMonitor for channel {}",
					log_funding_info!(monitor)
				);
				...
				if update_res.is_err() {
					log_error!(
						self.logger,
						"Failed to update ChannelMonitor for channel {}.",
						log_funding_info!(monitor)
					);
				}
				...
				let mut pending_monitor_updates =
					monitor_state.pending_monitor_updates.lock().unwrap();
				let persist_res = self.persister.update_persisted_channel(
					funding_txo,
					&Some(update),
					monitor,
					update_id,
				...
				if let Err(e) = persist_res {
					...
					log_error!(
						self.logger,
						"Failed to persist ChannelMonitor update for channel {}: {:?}",
						log_funding_info!(monitor),
						e
					);
				} else {
					log_trace!(
						self.logger,
						"Finished persisting ChannelMonitor update for channel {}",
						log_funding_info!(monitor)
					);
				}
				if update_res.is_err() {
	...
	fn release_pending_monitor_events(&self) -> Vec<MonitorEvent> {
		...
		for monitor_state in self.monitors.read().unwrap().values() {
			let is_pending_monitor_update = monitor_state.has_pending_chainsync_updates(
				&monitor_state.pending_monitor_updates.lock().unwrap(),
			);
			if is_pending_monitor_update
				&& monitor_state.last_chain_persist_height.load(Ordering::Acquire)
					+ LATENCY_GRACE_PERIOD_BLOCKS as usize
					> self.highest_chain_height.load(Ordering::Acquire)
			{
				...
				if is_pending_monitor_update {
					log_error!(
						self.logger,
						"A ChannelMonitor sync took longer than {} blocks to complete.",
						LATENCY_GRACE_PERIOD_BLOCKS
					);
					log_error!(
						self.logger,
						"   To avoid funds-loss, we are allowing monitor updates to be released."
					);
					log_error!(
						self.logger,
						"   This may cause duplicate payment events to be generated."
					);
				}
				pending_monitor_events
					.append(&mut monitor_state.monitor.get_and_clear_pending_monitor_events());
			}
		}
		pending_monitor_events
	}
...
mod tests {
	...
	use ::{
		expect_payment_path_successful, expect_payment_sent, expect_payment_sent_without_paths,
		get_event_msg,
	};
	use ::{
		get_htlc_update_msgs, get_local_commitment_txn, get_revoke_commit_msgs,
	...
	fn do_chainsync_pauses_events(block_timeout: bool) {
		...
		nodes[0].chain_monitor.chain_monitor.transactions_confirmed(
			&new_header,
			&[(0, &remote_txn[0]), (1, &remote_txn[1])],
		...
		nodes[0]
			.chain_monitor
			.chain_monitor
			.best_block_updated(&new_header, nodes[0].best_block_info().1 + 1);
		assert!(nodes[0].chain_monitor.release_pending_monitor_events().is_empty());
		...
		if block_timeout {
			...
			nodes[0].chain_monitor.chain_monitor.best_block_updated(
				&latest_header,
				nodes[0].best_block_info().1 + LATENCY_GRACE_PERIOD_BLOCKS,
			);
		} else {
			let persistences =
				chanmon_cfgs[0].persister.chain_sync_monitor_persistences.lock().unwrap().clone();
			for (funding_outpoint, update_ids) in persistences {
				for update_id in update_ids {
					nodes[0]
						.chain_monitor
						.chain_monitor
						.channel_monitor_updated(funding_outpoint, update_id)
						.unwrap();
				}
			}
		}
	...
	fn update_during_chainsync_fails_channel() {
		...
		check_closed_event!(
			...
			ClosureReason::ProcessingError {
				err: "Failed to persist ChannelMonitor update during chain sync".to_string()
			}
		);
		check_added_monitors!(nodes[0], 1);
	}
