...
use chain::keysinterface::{
	DelayedPaymentOutputDescriptor, KeysInterface, Sign, SpendableOutputDescriptor,
...
use ln::chan_utils;
use ln::chan_utils::{
	ChannelTransactionParameters, CounterpartyCommitmentSecrets, HTLCOutputInCommitment, HTLCType,
...
use util::ser::{
	MaybeReadable, OptionDeserWrapper, Readable, ReadableArgs, Writeable, Writer, U48,
};
...
impl<Signer: Sign> Writeable for ChannelMonitorImpl<Signer> {
	fn write<W: Writer>(&self, writer: &mut W) -> Result<(), Error> {
		...
		writer.write_all(&byte_utils::be64_to_array(
			self.counterparty_claimable_outpoints.len() as u64,
		))?;
		...
		writer.write_all(&byte_utils::be64_to_array(
			self.counterparty_commitment_txn_on_chain.len() as u64,
		))?;
		...
		writer.write_all(&byte_utils::be64_to_array(
			self.counterparty_hash_commitment_number.len() as u64,
		))?;
		...
		writer.write_all(&byte_utils::be64_to_array(
			self.onchain_events_awaiting_threshold_conf.len() as u64,
		))?;
...
impl<Signer: Sign> ChannelMonitor<Signer> {
	pub(crate) fn new(
		secp_ctx: Secp256k1<secp256k1::All>, keys: Signer, shutdown_script: Option<Script>,
		on_counterparty_tx_csv: u16, destination_script: &Script, funding_info: (OutPoint, Script),
		channel_parameters: &ChannelTransactionParameters, funding_redeemscript: Script,
		...
		let counterparty_payment_script = Builder::new()
			.push_opcode(opcodes::all::OP_PUSHBYTES_0)
			.push_slice(&payment_key_hash[..])
			.into_script();
	...
	pub(crate) fn provide_latest_counterparty_commitment_tx<L: Deref>(
		...
		commitment_number: u64, their_revocation_point: PublicKey, logger: &L,
	) where
		L::Target: Logger,
	{
	...
	fn provide_latest_holder_commitment_tx(
		...
		self.inner
			.lock()
			.unwrap()
			.provide_latest_holder_commitment_tx(holder_commitment_tx, htlc_outputs)
			.map_err(|_| ())
	}
	...
	pub fn load_outputs_to_watch<F: Deref>(&self, filter: &F)
	where
	...
	pub fn block_connected<B: Deref, F: Deref, L: Deref>(
		&self, header: &BlockHeader, txdata: &TransactionData, height: u32, broadcaster: B,
		fee_estimator: F, logger: L,
	...
	pub fn block_disconnected<B: Deref, F: Deref, L: Deref>(
		&self, header: &BlockHeader, height: u32, broadcaster: B, fee_estimator: F, logger: L,
	) where
		B::Target: BroadcasterInterface,
		F::Target: FeeEstimator,
		L::Target: Logger,
	...
	pub fn get_claimable_balances(&self) -> Vec<Balance> {
					...
					if let Some(htlc_input_idx) = htlc.transaction_output_index {
						if let Some(conf_thresh) =
							us.onchain_events_awaiting_threshold_conf.iter().find_map(|event| {
								...
								{
									if descriptor.outpoint.index as u32 == htlc_input_idx {
										Some(event.confirmation_threshold())
									} else {
										None
									}
								} else {
							...
							.iter()
							.any(|v| v.input_idx == htlc_input_idx)
						{
							...
							debug_assert!(
								$holder_commitment || us.funding_spend_confirmed.is_some()
							);
						} else if htlc.offered == $holder_commitment {
							// If the payment was outbound, check if there's an HTLCUpdate
							// indicating we have spent this HTLC with a timeout, claiming it back
							// and awaiting confirmations on it.
							let htlc_update_pending = us
								.onchain_events_awaiting_threshold_conf
								.iter()
								.find_map(|event| {
							...
							let htlc_spend_pending = us
								.onchain_events_awaiting_threshold_conf
								.iter()
								.find_map(|event| {
									if let OnchainEvent::HTLCSpendConfirmation {
										input_idx,
										preimage,
										..
									} = event.event
		...
		if let Some(txid) = confirmed_txid {
				...
				|| Some(txid) == us.prev_counterparty_commitment_txid
			{
				walk_htlcs!(
					false,
					us.counterparty_claimable_outpoints.get(&txid).unwrap().iter().map(|(a, _)| a)
				);
				...
				walk_htlcs!(
					true,
					us.current_holder_commitment_tx.htlc_outputs.iter().map(|(a, _, _)| a)
				);
				if let Some(conf_thresh) = pending_commitment_tx_conf_thresh {
					res.push(Balance::ClaimableAwaitingConfirmations {
						claimable_amount_satoshis: us
	...
	pub(crate) fn get_pending_outbound_htlcs(&self) -> HashMap<HTLCSource, HTLCOutputInCommitment> {
			...
			($holder_commitment: expr, $htlc_iter: expr) => {
				for (htlc, source) in $htlc_iter {
					if us
						.htlcs_resolved_on_chain
						.iter()
						.any(|v| Some(v.input_idx) == htlc.transaction_output_index)
					{
		...
		if let Some(txid) = confirmed_txid {
			...
			macro_rules! walk_counterparty_commitment {
				($txid: expr) => {
					if let Some(ref latest_outpoints) =
						us.counterparty_claimable_outpoints.get($txid)
					{
...
impl<Signer: Sign> ChannelMonitorImpl<Signer> {
			...
			Some(old_points) => {
				if old_points.0 == commitment_number + 1 {
					self.their_cur_revocation_points =
						Some((old_points.0, old_points.1, Some(their_revocation_point)));
				} else if old_points.0 == commitment_number + 2 {
					if let Some(old_second_point) = old_points.2 {
						self.their_cur_revocation_points = Some((
							old_points.0 - 1,
							old_second_point,
							Some(their_revocation_point),
						));
					} else {
						self.their_cur_revocation_points =
							Some((commitment_number, their_revocation_point, None));
					}
				} else {
					self.their_cur_revocation_points =
						Some((commitment_number, their_revocation_point, None));
		...
		L::Target: Logger,
	{
		...
		macro_rules! claim_htlcs {
			($commitment_number: expr, $txid: expr) => {
				let htlc_claim_reqs =
					self.get_counterparty_htlc_output_claim_reqs($commitment_number, $txid, None);
		...
		if self.broadcasted_holder_revokable_script.is_some() {
			...
			if let Some(ref tx) = self.prev_holder_signed_commitment_tx {
				let (claim_reqs, _) =
					self.get_broadcasted_holder_claims(&tx, self.best_block.height());
				self.onchain_tx_handler.update_claims_view(
					&Vec::new(),
					claim_reqs,
					self.best_block.height(),
	...
	pub(crate) fn broadcast_latest_holder_commitment_txn<B: Deref, L: Deref>(
		&mut self, broadcaster: &B, logger: &L,
	) where
		B::Target: BroadcasterInterface,
		L::Target: Logger,
	{
	...
	pub fn update_monitor<B: Deref, F: Deref, L: Deref>(
		&mut self, updates: &ChannelMonitorUpdate, broadcaster: &B, fee_estimator: &F, logger: &L,
	) -> Result<(), ()>
	where
		B::Target: BroadcasterInterface,
		F::Target: FeeEstimator,
	...
	fn check_spend_counterparty_transaction<L: Deref>(
		&mut self, tx: &Transaction, height: u32, logger: &L,
	) -> (Vec<PackageTemplate>, TransactionOutputs)
	where
		L::Target: Logger,
	{
		...
		let commitment_number = 0xffffffffffff
			- ((((tx.input[0].sequence as u64 & 0xffffff) << 3 * 8)
		...
		if commitment_number >= self.get_min_seen_secret() {
			...
			let per_commitment_point =
				PublicKey::from_secret_key(&self.secp_ctx, &per_commitment_key);
			let revocation_pubkey = ignore_error!(chan_utils::derive_public_revocation_key(
				&self.secp_ctx,
			...
			fail_unbroadcast_htlcs!(
				...
				"counterparty",
				height,
				per_commitment_data.iter().map(|(a, b)| (a, b.as_ref().map(|b| b.as_ref()))),
				logger
			);

			let htlc_claim_reqs = self.get_counterparty_htlc_output_claim_reqs(
				commitment_number,
				commitment_txid,
				Some(tx),
			);
	...
	fn get_counterparty_htlc_output_claim_reqs(
	...
	) -> (Vec<PackageTemplate>, Option<(Script, PublicKey, PublicKey)>) {
		...
		if self.current_holder_commitment_tx.txid == commitment_txid {
			...
			if holder_tx.txid == commitment_txid {
				...
				fail_unbroadcast_htlcs!(
					...
					"previous holder",
					height,
					holder_tx.htlc_outputs.iter().map(|(a, _, c)| (a, c.as_ref())),
					logger
				);
			}
		}
	...
	pub fn get_latest_holder_commitment_txn<L: Deref>(&mut self, logger: &L) -> Vec<Transaction>
	where
		L::Target: Logger,
	{
		...
		let commitment_tx =
			self.onchain_tx_handler.get_fully_signed_holder_tx(&self.funding_redeemscript);
		...
		for htlc in self.current_holder_commitment_tx.htlc_outputs.iter() {
			if let Some(vout) = htlc.0.transaction_output_index {
				...
				if let Some(htlc_tx) = self
					.onchain_tx_handler
					.get_fully_signed_htlc_tx(&::bitcoin::OutPoint { txid, vout }, &preimage)
				{
	...
	fn unsafe_get_latest_holder_commitment_txn<L: Deref>(&mut self, logger: &L) -> Vec<Transaction>
	where
		L::Target: Logger,
	{
		...
		for htlc in self.current_holder_commitment_tx.htlc_outputs.iter() {
			if let Some(vout) = htlc.0.transaction_output_index {
				...
				if let Some(htlc_tx) = self
					.onchain_tx_handler
					.unsafe_get_fully_signed_htlc_tx(&::bitcoin::OutPoint { txid, vout }, &preimage)
				{
					holder_transactions.push(htlc_tx);
				}
			}
	...
	pub fn block_connected<B: Deref, F: Deref, L: Deref>(
		&mut self, header: &BlockHeader, txdata: &TransactionData, height: u32, broadcaster: B,
		fee_estimator: F, logger: L,
	) -> Vec<TransactionOutputs>
	where
		B::Target: BroadcasterInterface,
	...
	fn best_block_updated<B: Deref, F: Deref, L: Deref>(
		&mut self, header: &BlockHeader, height: u32, broadcaster: B, fee_estimator: F, logger: L,
	) -> Vec<TransactionOutputs>
	where
		B::Target: BroadcasterInterface,
		F::Target: FeeEstimator,
			...
			if tx.input.len() == 1 {
					...
					&& prevout.vout == self.funding_info.0.index as u32
				{
					...
					log_info!(
						...
						log_bytes!(self.funding_info.0.to_channel_id()),
						tx.txid()
					);
					self.funding_spend_seen = true;
					if (tx.input[0].sequence >> 8 * 3) as u8 == 0x80
						&& (tx.lock_time >> 8 * 3) as u8 == 0x20
					{
						let (mut new_outpoints, new_outputs) =
						...
						if new_outpoints.is_empty() {
							if let Some((mut new_outpoints, new_outputs)) =
								self.check_spend_holder_transaction(&tx, height, &logger)
							{
								if !new_outputs.1.is_empty() {
									watch_outputs.push(new_outputs);
								}
					...
					if let Some(&commitment_number) =
						self.counterparty_commitment_txn_on_chain.get(&prevout.txid)
					{
						let (mut new_outpoints, new_outputs_option) = self
							.check_spend_counterparty_htlc(&tx, commitment_number, height, &logger);
						claimable_outpoints.append(&mut new_outpoints);
	...
	fn block_confirmed<B: Deref, F: Deref, L: Deref>(
		&mut self, conf_height: u32, txn_matched: Vec<&Transaction>,
		mut watch_outputs: Vec<TransactionOutputs>, mut claimable_outpoints: Vec<PackageTemplate>,
		broadcaster: &B, fee_estimator: &F, logger: &L,
	) -> Vec<TransactionOutputs>
	where
		B::Target: BroadcasterInterface,
		F::Target: FeeEstimator,
		L::Target: Logger,
	{
		...
		if should_broadcast {
			...
			self.pending_monitor_events
				.push(MonitorEvent::CommitmentTxConfirmed(self.funding_info.0));
			let commitment_tx =
				self.onchain_tx_handler.get_fully_signed_holder_tx(&self.funding_redeemscript);
			self.holder_tx_signed = true;
		...
		let unmatured_htlcs: Vec<_> = self
			.onchain_events_awaiting_threshold_conf
			.iter()
		...
		for entry in onchain_events_reaching_threshold_conf.drain(..) {
			match entry.event {
				OnchainEvent::HTLCUpdate {
					...
					log_debug!(
						logger,
						"HTLC {} failure update has got enough confirmations to be passed upstream",
				...
				OnchainEvent::MaturingOutput { descriptor } => {
					...
					self.pending_events.push(Event::SpendableOutputs { outputs: vec![descriptor] });
				}
				OnchainEvent::HTLCSpendConfirmation { input_idx, preimage, .. } => {
					self.htlcs_resolved_on_chain
						.push(IrrevocablyResolvedHTLC { input_idx, payment_preimage: preimage });
				}
	...
	fn transaction_unconfirmed<B: Deref, F: Deref, L: Deref>(
		&mut self, txid: &Txid, broadcaster: B, fee_estimator: F, logger: L,
	) where
		B::Target: BroadcasterInterface,
		F::Target: FeeEstimator,
		L::Target: Logger,
							...
							if _script_pubkey.is_v0_p2wsh() {
								assert_eq!(
									&bitcoin::Address::p2wsh(
										&Script::from(input.witness.last().unwrap().clone()),
	...
	fn should_broadcast_holder_commitment_txn<L: Deref>(&self, logger: &L) -> bool
	where
		L::Target: Logger,
	{
	...
	fn is_resolving_htlc_output<L: Deref>(&mut self, tx: &Transaction, height: u32, logger: &L)
	where
		L::Target: Logger,
	{
		'outer_loop: for input in &tx.input {
			let mut payment_data = None;
			let revocation_sig_claim = (input.witness.len() == 3
				&& HTLCType::scriptlen_to_htlctype(input.witness[2].len())
					== Some(HTLCType::OfferedHTLC)
				&& input.witness[1].len() == 33)
				|| (input.witness.len() == 3
					&& HTLCType::scriptlen_to_htlctype(input.witness[2].len())
						== Some(HTLCType::AcceptedHTLC)
					&& input.witness[1].len() == 33);
			let accepted_preimage_claim = input.witness.len() == 5
				&& HTLCType::scriptlen_to_htlctype(input.witness[4].len())
					== Some(HTLCType::AcceptedHTLC);
			#[cfg(not(fuzzing))]
			let accepted_timeout_claim = input.witness.len() == 3
				&& HTLCType::scriptlen_to_htlctype(input.witness[2].len())
					== Some(HTLCType::AcceptedHTLC)
				&& !revocation_sig_claim;
			let offered_preimage_claim = input.witness.len() == 3
				&& HTLCType::scriptlen_to_htlctype(input.witness[2].len())
					== Some(HTLCType::OfferedHTLC)
				&& !revocation_sig_claim;
			#[cfg(not(fuzzing))]
			let offered_timeout_claim = input.witness.len() == 5
				&& HTLCType::scriptlen_to_htlctype(input.witness[4].len())
					== Some(HTLCType::OfferedHTLC);
			...
			macro_rules! check_htlc_valid_counterparty {
				($counterparty_txid: expr, $htlc_output: expr) => {
					if let Some(txid) = $counterparty_txid {
						for &(ref pending_htlc, ref pending_source) in
							self.counterparty_claimable_outpoints.get(&txid).unwrap()
						{
				...
				self.counterparty_claimable_outpoints.get(&input.previous_output.txid)
			{
				scan_commitment!(
					htlc_outputs
						.iter()
						.map(|&(ref a, ref b)| (a, (b.as_ref().clone()).map(|boxed| &**boxed))),
	...
	fn is_paying_spendable_output<L: Deref>(&mut self, tx: &Transaction, height: u32, logger: &L)
	where
		L::Target: Logger,
	{
		let mut spendable_output = None;
		for (i, outp) in tx.output.iter().enumerate() {
			// There is max one spendable output for any channel tx, including ones generated by us
				...
				self.broadcasted_holder_revokable_script
			{
				if broadcasted_holder_revokable_script.0 == outp.script_pubkey {
					spendable_output = Some(SpendableOutputDescriptor::DelayedPaymentOutput(
			...
			if self.counterparty_payment_script == outp.script_pubkey {
				spendable_output = Some(SpendableOutputDescriptor::StaticPaymentOutput(
					StaticPaymentOutputDescriptor {
						outpoint: OutPoint { txid: tx.txid(), index: i as u16 },
						output: outp.clone(),
						channel_keys_id: self.channel_keys_id,
...
impl<Signer: Sign, T: Deref, F: Deref, L: Deref> chain::Confirm
	...
	for (BlockHash, ChannelMonitor<Signer>)
{
	fn read<R: io::Read>(reader: &mut R, keys_manager: &'a K) -> Result<Self, DecodeError> {
		...
		let mut counterparty_claimable_outpoints = HashMap::with_capacity(cmp::min(
		...
		let mut counterparty_commitment_txn_on_chain = HashMap::with_capacity(cmp::min(
		...
		let mut counterparty_hash_commitment_number = HashMap::with_capacity(cmp::min(
		...
		let mut payment_preimages =
			HashMap::with_capacity(cmp::min(payment_preimages_len as usize, MAX_ALLOC_SIZE / 32));
		...
		let mut outputs_to_watch = HashMap::with_capacity(cmp::min(
			outputs_to_watch_len as usize,
			MAX_ALLOC_SIZE
				/ (mem::size_of::<Txid>() + mem::size_of::<u32>() + mem::size_of::<Vec<Script>>()),
		));
		for _ in 0..outputs_to_watch_len {
			...
			let mut outputs = Vec::with_capacity(cmp::min(
				...
				MAX_ALLOC_SIZE / (mem::size_of::<u32>() + mem::size_of::<Script>()),
			));
		...
		let onchain_tx_handler: OnchainTxHandler<Signer> =
			ReadableArgs::read(reader, keys_manager)?;
...
mod tests {
	...
	use util::test_utils::{TestBroadcaster, TestFeeEstimator, TestLogger};
	use ::{
		check_added_monitors, check_closed_broadcast, check_closed_event, check_spends,
		get_local_commitment_txn, get_monitor, get_route_and_payment_hash, unwrap_send_err,
	};
	...
	fn do_test_funding_spend_refuses_updates(use_local_txn: bool) {
		...
		check_closed_event!(
			nodes[1],
			1,
			ClosureReason::ProcessingError { err: "ChannelMonitor storage failure".to_string() }
		);
		...
		let mut replay_update =
			monitor_updates.get(&channel.2).unwrap().iter().rev().skip(1).next().unwrap().clone();
	...
	fn test_prune_preimages() {
		...
		let dummy_tx =
			Transaction { version: 0, lock_time: 0, input: Vec::new(), output: Vec::new() };
		...
		monitor
			.provide_latest_holder_commitment_tx(
				HolderCommitmentTransaction::dummy(),
				preimages_to_holder_htlcs!(preimages[0..10]),
			)
			.unwrap();
		...
		monitor
			.provide_latest_holder_commitment_tx(
				HolderCommitmentTransaction::dummy(),
				preimages_to_holder_htlcs!(preimages[0..5]),
			)
			.unwrap();
		secret[0..32].clone_from_slice(
		...
		monitor
			.provide_latest_holder_commitment_tx(
				HolderCommitmentTransaction::dummy(),
				preimages_to_holder_htlcs!(preimages[0..3]),
			)
			.unwrap();
		secret[0..32].clone_from_slice(
	...
	fn test_claim_txn_weight_computation() {
		...
		macro_rules! sign_input {
			($sighash_parts: expr, $idx: expr, $amount: expr, $weight: expr, $sum_actual_sigs: expr, $opt_anchors: expr) => {
				...
				let redeem_script = if *$weight == WEIGHT_REVOKED_OUTPUT {
					chan_utils::get_revokeable_redeemscript(&pubkey, 256, &pubkey)
				} else {
				...
				let sighash = hash_to_message!(
					&$sighash_parts.signature_hash($idx, &redeem_script, $amount, SigHashType::All)
						[..]
				);
		...
		let txid =
			Txid::from_hex("56944c5d3f98413ef45cf54545538103cc9f298e0575820ad3591376e2e0f65d")
				.unwrap();
		...
		for &opt_anchors in [false, true].iter() {
			...
			claim_tx.output.push(TxOut { script_pubkey: script_pubkey.clone(), value: 0 });
			let base_weight = claim_tx.get_weight();
			let inputs_weight = vec![
				WEIGHT_REVOKED_OUTPUT,
				weight_revoked_offered_htlc(opt_anchors),
				weight_revoked_offered_htlc(opt_anchors),
				weight_revoked_received_htlc(opt_anchors),
			];
			...
			assert_eq!(
				base_weight + inputs_total_weight as usize,
				claim_tx.get_weight() + /* max_length_sig */ (73 * inputs_weight.len() - sum_actual_sigs)
			);
		}
		...
		for &opt_anchors in [false, true].iter() {
			...
			let base_weight = claim_tx.get_weight();
			let inputs_weight = vec![
				weight_offered_htlc(opt_anchors),
				weight_received_htlc(opt_anchors),
				weight_received_htlc(opt_anchors),
				weight_received_htlc(opt_anchors),
			];
			let mut inputs_total_weight = 2; // count segwit flags
		...
		for &opt_anchors in [false, true].iter() {
			...
			assert_eq!(
				base_weight + inputs_total_weight as usize,
				claim_tx.get_weight() + /* max_length_isg */ (73 * inputs_weight.len() - sum_actual_sigs)
			);
		}
	}
